...
import {dragRegion,dragRegionAspect,fitRegionAspect,FULL_REGION,sanitizeRegion,sanitizeRegionStore} from '../web-host/wwwroot/target-region.mjs';
...
assert.deepEqual(fitRegionAspect(FULL_REGION,.5),{x:.25,y:0,width:.5,height:1});
assert.deepEqual(fitRegionAspect(FULL_REGION,2),{x:0,y:.25,width:1,height:.5});
assert.deepEqual(dragRegionAspect({x:.25,y:.25,width:.5,height:.5},'se',.2,.1,1),{x:.25,y:.25,width:.7,height:.7});
assert.deepEqual(dragRegionAspect({x:.25,y:.25,width:.5,height:.5},'nw',.4,.4,1),{x:.65,y:.65,width:.1,height:.1});
assert.deepEqual(dragRegionAspect({x:.25,y:.25,width:.5,height:.5},'move',.5,-.5,1),{x:.5,y:0,width:.5,height:.5});
console.log('20 Windows target-region checks passed');
