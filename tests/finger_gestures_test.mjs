...
events=[];finger=make('touch');
finger.down(1,50,50,0);
finger.move(1,52,51,10);
assert.equal(events.filter(e=>e.phase===1).length,0,'stationary touch suppresses jitter moves');
finger.move(1,65,50,20);
assert.equal(events.filter(e=>e.phase===1).length,1,'intentional move emits touch move');
console.log('Finger gesture checks passed: move, tap, double-click, drag, scroll, right-click, sensitivity, five-touch, jitter suppression and cancellation');
