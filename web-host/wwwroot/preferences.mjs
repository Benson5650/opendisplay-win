export function sanitizePreferences(value) {
  ...
  const result={};
  const options={mode:['pen','mirror','extend'],mapping:['preserve','stretch'],fps:['30','60'],quality:['fast','balanced','high'],pressureCurve:['linear','soft','firm'],trackpadSensitivity:['slow','normal','fast'],penBackground:['dark','black','white']};
  for(const [key,allowed] of Object.entries(options))if(allowed.includes(value[key]))result[key]=value[key];
  for(const key of ['showActiveArea','showHover','showDebugLog'])if(typeof value[key]==='boolean')result[key]=value[key];
