export function sanitizePreferences(value) {
  ...
  const result={};
  const options={mode:['pen','mirror','extend'],mapping:['preserve','stretch'],fps:['30','60'],quality:['fast','balanced','high'],pressureCurve:['linear','soft','firm'],trackpadSensitivity:['slow','normal','fast'],penBackground:['dark','black','white'],activeAreaScale:['1','0.9','0.8','0.7'],resolutionScale:['1','0.75','0.5']};
  for(const [key,allowed] of Object.entries(options))if(allowed.includes(value[key]))result[key]=value[key];
  for(const key of ['showActiveArea','showHover','showDebugLog','hideCursor'])if(typeof value[key]==='boolean')result[key]=value[key];
