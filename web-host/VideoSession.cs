...
internal sealed class VideoSession : IDisposable
{
    ...
    public VideoSession(string target, uint fps, ulong generation, string token, nint ownedDisplay = 0, uint bitrate = 12_000_000, double scale = 1.0)
    {
        Generation = generation; DeviceToken = token;
        handle = Native.od_video_create(target, fps, bitrate, scale);
